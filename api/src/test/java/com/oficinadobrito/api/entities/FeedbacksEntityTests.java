...
class FeedbacksEntityTests {
    ...
    @DisplayName("Update Feedback from DTO")
    @Test
    void testUpdateFeedbackFromDto() {
        // Given - Arrange
        UpdateFeedbackDto dto = new UpdateFeedbackDto(GenerateUUID.generateUUID(),"@netox" , 4.2,"Excelente serviço!",GenerateUUID.generateUUID(), GenerateBigInteger.generateCustomRandom());

        // When - Act
        Feedback updatedFeedback = Feedback.updateDtoToEntity(dto);

        // Then - Assert
        Assertions.assertEquals(dto.comentario(), updatedFeedback.getComentario(), "The comentario was not correctly updated from DTO");
        Assertions.assertEquals(dto.stars(), updatedFeedback.getStars(), "The stars value was not correctly updated from DTO");
    }
}
