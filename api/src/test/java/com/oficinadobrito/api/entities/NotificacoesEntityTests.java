...
class NotificacoesEntityTests {
    ...
    @DisplayName("Update Notificacao from DTO")
    @Test
    void testUpdateNotificacaoFromDto() {
        // Given - Arrange
        UpdateNotificacaoDto dto = new UpdateNotificacaoDto(novoAssunto, novaDescricao, novoStatus);

        // When - Act
        Notificacao updatedNotificacao = Notificacao.updateDtoToEntity(dto);

        // Then - Assert
        assertEquals(dto.assunto(), updatedNotificacao.getAssunto(), "The assunto was not correctly updated from DTO");
        assertEquals(dto.descricao(), updatedNotificacao.getDescricao(), "The descricao was not correctly updated from DTO");
        assertEquals(dto.status(), updatedNotificacao.getStatus(), "The status was not correctly updated from DTO");
    }
